<template>
  <div id="g-7_-Habits-R-box" class="ai2html">

    <!-- Artboard: Artboard_3 -->
    <div id="g-7_-Habits-R-Artboard_3" class="g-artboard" style="max-width: 522px;max-height: 684px" data-aspect-ratio="0.763" data-min-width="0">
      <div style="padding: 0 0 131.0345% 0;"></div>
      <img id="g-7_-Habits-R-Artboard_3-img" class="g-aiImg" alt="" src="./7_-Habits-R-Artboard_3.png"/>
    </div>

  </div>
</template>

<script>
export default {
  name: "weeklyHabits2",
  props:{
    data:{
      type: Object,
    },
  }
}
</script>

<style media="screen,print">
p{
  font-family: 'Kontour Type - Kopius-Light';
}
#g-7_-Habits-R-box ,
#g-7_-Habits-R-box .g-artboard {
  margin:0 auto;
}
#g-7_-Habits-R-box p {
  margin:0;
}
#g-7_-Habits-R-box .g-aiAbs {
  position:absolute;
}
#g-7_-Habits-R-box .g-aiImg {
  position:absolute;
  top:0;
  display:block;
  width:100% !important;
}
#g-7_-Habits-R-box .g-aiSymbol {
  position: absolute;
  box-sizing: border-box;
}
#g-7_-Habits-R-box .g-aiPointText p { white-space: nowrap; }
#g-7_-Habits-R-Artboard_3 {
  position:relative;
  overflow:hidden;
}

</style>

